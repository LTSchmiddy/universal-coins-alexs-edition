...
public class UniversalCoins {
	...
	public static Boolean atmGivesEnderCard;
	public static Boolean atmGivesEnderCard_default = false;

	// 0 = normal mode, 1 = whitelist mode, 2 = blacklist mode
	public static Integer shopStockingMode = 0;
	public static Integer shopStockingMode_default = 1;

	public static String shopListFilename = "UC_shop_whitelist.cfg";


	public static String[] shopItemWhitelist;
	public static String[] shopItemBlacklist;
	...
	public void preInit(FMLPreInitializationEvent event) {
		...
		//Added By Alex:
		Property atmGivesEnderCardProperty  = config.get("Misc", "Atm Gives Ender Card", true);
		shopMaxPriceProperty.comment = "If true, ATMs will give out Ender Cards instead of regular ATM cards when making a new account, replacing an old card, etc";
		atmGivesEnderCard = atmGivesEnderCardProperty.getBoolean(atmGivesEnderCard_default);

//		Property shopStockingModeProperty  = config.get("Misc", "Shop Stocking Mode", shopStockingMode_default);
//		shopStockingModeProperty.comment = "0 = normal mode, 1 = whitelist mode, 2 = blacklist mode. Whitelist and blacklist";
//		shopStockingMode = shopStockingModeProperty.getInt(shopStockingMode_default);
//
//		if (shopStockingMode < 0 || shopStockingMode > 2) {
//			System.out.println("UC: not a valid option for shopStockingMode. reverting to mode 0");
//			shopStockingMode = 0;
//		}
//
//		String[] defaultItemList = new String[0];
////		String[] defaultItemList = {Item.getItemById(322).getUnlocalizedName()};
////		String[] defaultItemList = {Item.getItemById(322).getUnlocalizedName(), Item.getItemById(1).getUnlocalizedName(), Item.getItemById(2).getUnlocalizedName(), Item.getItemById(3).getUnlocalizedName()};
//
//		Property shopItemWhitelistProperty  = config.get("z: Shop Item Lists", "Whitelist", defaultItemList);
//		shopItemWhitelistProperty.comment = "In Whilelist mode, Universal Coins can only stock Items from this list.";
//		shopItemWhitelist = shopItemWhitelistProperty.getStringList();
//		printItemWhitelist();
//		UCItemPricer.UC_Whitelist = shopItemWhitelist;

//
//
//		Property shopItemBlacklistProperty  = config.get("z: Shop Item Lists", "Blacklist", new String[0]);
//		shopItemBlacklistProperty.comment = "In Blacklist mode, Universal Coins can will not stock shops with Items on this list.";
//		shopItemWhitelist = shopItemBlacklistProperty.getStringList();
...
//	public static String[] buildItemList() {
//		String[] retVal = null;
//
//		ArrayList<String> assembleArray = new ArrayList<>();
//
//
//
//
...
//	public static Boolean itemInShopWhitelist(Item thisItem){
//		String itemName = thisItem.getUnlocalizedName();
//
//		for (int i = 0; i < shopItemWhitelist.length; i++) {
//			if (shopItemWhitelist[i].equalsIgnoreCase(itemName)) {
//				return true;
//			}
//		}
//		return false;
//	}
	...
	public static Boolean itemInShopWhitelist(String itemName){

		for (int i = 0; i < shopItemWhitelist.length; i++) {
			if (shopItemWhitelist[i].equals(itemName)) {
				return true;
			}
		}
		return false;
	...
	public static void printItemWhitelist() {
		String whitelistMessage = "\n*******************\nShop Item Whitelist:\n";

		for (int i = 0; i < shopItemWhitelist.length; i++) {
			whitelistMessage += shopItemWhitelist[i] + "\n";
		}

		whitelistMessage += "\n\n*******************\n";

		System.out.println(whitelistMessage);
		System.out.println("Shop Item Whitelist Length: " + shopItemWhitelist.length);
	}
}
