...
public class UCItemPricer {
	...
	public static String[] UC_Whitelist;
	...
	public ItemStack getRandomPricedStack() {
		return getRandomPricedStack(false);
	}

	public ItemStack getRandomPricedStack(Boolean shopMode) {
		List keys = new ArrayList(ucPriceMap.keySet());
		ItemStack stack = null;
		while (stack == null) {
//			String baseKeyName = keys.get(random.nextInt(keys.size())).toString();
//			String keyName = keys.get(random.nextInt(keys.size())).toString();

			//Added by Alex:
			String keyName;
//			if ((shopMode) && (UniversalCoins.shopStockingMode == 1)) {
////				UniversalCoins.printItemWhitelist();
//
//
////				int nextSelection = random.nextInt(UniversalCoins.shopItemWhitelist.length);
//// 				keyName = UniversalCoins.shopItemWhitelist[nextSelection];
//
////				printItemWhitelist();
//				int nextSelection = random.nextInt(UC_Whitelist.length);
////				keyName = UC_Whitelist[nextSelection];
//				keyName = "item.pickaxeDiamond.0";
//
//
//			} else {
				keyName = keys.get(random.nextInt(keys.size())).toString();
...
//			}
			...
			if (price > 0) {
//				System.out.println(keyName + " price: " + keyName);
				...
				if (!Character.isDigit(keyName.charAt(keyName.length() - 1))) {
					...
					if (item != null) {
						...
						System.out.println("couldn't find item in game registry: " + itemName + "   keyname was: " + keyName);
				...
				System.out.println("couldn't find item in price table: " + keyName);
	...
	public static void printItemWhitelist() {
		...
		for (int i = 0; i < UC_Whitelist.length; i++) {
			whitelistMessage += UC_Whitelist[i] + "\n";
		}

		whitelistMessage += "\n\n*******************\n";

		System.out.println(whitelistMessage);
		System.out.println("Shop Item Whitelist Length: " + UC_Whitelist.length);
	}
}
